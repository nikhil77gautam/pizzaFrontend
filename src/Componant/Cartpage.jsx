...
const Cartpage = () => {
  ...
  const updateCartQuantity = async (id, newQuantity) => {
    try {
      await axios.put(`http://localhost:8000/updatecartQuantity/${customerId}/${id}`, {
  ...
  const removeFromCart = async (id) => {
    try {
      await axios.post("http://localhost:8000/deletecartpizza", {
        customerId,
        Id: id
      }, {
        headers: {
  ...
  return (
    <div className="min-h-screen bg-cover bg-center flex justify-center items-center py-10 px-4" 
      style={{ backgroundImage: 'url(https://th.bing.com/th/id/OIP.sdUT21iwISmJm4IFVSpzGgHaOl?pid=ImgDet&w=161&h=316&c=7)' }}>
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-3xl">
        <h1 className="text-3xl font-bold mb-6 text-center text-orange-600">Your Cart</h1>
        
        {loading ? (
          <p className="text-center text-gray-500">Loading...</p>
        ) :  (
          <>
            {!userCart || !userCart.cart || (!userCart.cart.pizzas.length && !userCart.cart.meals.length) ? (
              <p className="text-center text-gray-500">Your cart is empty.</p>
            ) : (
              <div>
                {userCart.cart.pizzas && userCart.cart.pizzas.length > 0 && (
                  <div className="mb-6">
                    <h2 className="text-2xl font-bold text-gray-800 mb-4">Pizzas</h2>
                    {userCart.cart.pizzas.map((item) => (
                      <div key={item.pizza._id} className="flex items-center border-b py-4">
                        <img
                          src={`http://localhost:8000/${item.pizza.image}`}
                          alt={item.pizza.title}
                          className="w-20 h-20 rounded-lg mr-4"
                        />
                        <div className="flex-1">
                          <h2 className="text-xl font-semibold text-gray-800">{item.pizza.title}</h2>
                          <p className="text-gray-600">Quantity: {quantities[item.pizza._id] || item.quantity}</p>
                        </div>
                        ...
                        <p className="text-orange-600 font-semibold mx-4">₹{item.pizza.price}</p>
                        <button
                ...
                {userCart.cart.meals && userCart.cart.meals.length > 0 && (
                  <div>
                    ...
                    {userCart.cart.meals.map((item) => (
                      <div key={item.meal._id} className="flex items-center border-b py-4">
                        <img
                          src={`http://localhost:8000/${item.meal.image}`}
                          alt={item.meal.title}
                          className="w-20 h-20 rounded-lg mr-4"
                        />
                        <div className="flex-1">
                          <h2 className="text-xl font-semibold text-gray-800">{item.meal.title}</h2>
                          <p className="text-gray-600">Quantity: {quantities[item.meal._id] || item.quantity}</p>
                        </div>
                        <div className="flex items-center bg-orange-500 rounded-lg">
                          <button
                            className="p-2 text-white font-bold w-10 h-10"
                        ...
                        <p className="text-orange-600 font-semibold mx-4">₹{item.meal.price}</p>
                        <button
                          className="text-red-600 hover:text-red-800"
                ...
                <div className="mt-6 flex justify-between items-center text-xl">
                  ...
                  <p className="font-semibold text-orange-600">₹{userCart.totalAmount}</p>
                </div>
